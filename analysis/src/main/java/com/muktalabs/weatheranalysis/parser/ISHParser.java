package com.muktalabs.weatheranalysis.parser;
...
public class ISHParser {
	...
	static NumberFormat fmt03 = new DecimalFormat("000"); // 3-char int (keep
															// leading zeros)
	...
	public static String parse(String line) {
		...
		try {
			...
			if (iREM_IndexOf == -1) {
				iREM_IndexOf = 9999; // If no REM section then set to
										// high value
			}
			...
			if (sCDS_Wban.equals("99999")) // Show WBAN=99999 as missing
											// "*****" in output file
			{
				sCDS_Wban = "*****";
			}
			// Build Control Data Section
			sControlSection = sCDS_ID + " " + sCDS_Wban + " " + sCDS_Year
					+ sCDS_Month + sCDS_Day + sCDS_Hour + sCDS_Minute;
			...
			sMandatorySection = sMDS_Dir + " " + sMDS_Spd + " " + sOC1_Gus
					+ " " + sMDS_Clg + " " + sGF1_Skc + " " + sGF1_Low + " "
					+ sGF1_Med + " " + sGF1_Hi + " " + sMDS_Vsb + " "
					+ sWW1234[3] + " " + sWW1234[2] + " " + sWW1234[1] + " "
					+ sWW1234[0] + " " + sAW1234[3] + " " + sAW1234[2] + " "
					+ sAW1234[1] + " " + sAW1234[0] + " " + sAY1_Pw + " "
					+ sMDS_Temp + " " + sMDS_Dewp + " " + sMDS_Slp + " "
					+ sMA1_Alt + " " + sMA1_Stp + " " + sMaxTemp + " "
					+ sMinTemp + " " + sPcp01 + sPcp01t + sPcp06 + sPcp06t
					+ sPcp24 + sPcp24t + sPcp12 + sPcp12t + sAJ1_Sd;

			sOutputRecord = sControlSection + " " + sMandatorySection; // Put
																		// it
																		// all
																		// together
		...
		return sOutputRecord; // record

	}

	public static void main(String[] args) {
		String a[]={"0089010010999992014010101004+70933-008667FM-12+000999999V0203601N004019999999N999999999-00041-00401101211ADDMA1999990101091MD1810071+9990REMSYN04801001 46/// /3604 11004 21040 30109 40121 58007=",
				"0089010010999992014010102004+70933-008667FM-12+000999999V0203401N010019999999N999999999-00031-00291101181ADDMA1999990101061MD1810041+9990REMSYN04801001 46/// /3410 11003 21029 30106 40118 58004=",
				"0089010010999992014010104004+70933-008667FM-12+000999999V0203501N010019999999N999999999-00031-00191101101ADDMA1999990100981MD1810101+9990REMSYN04801001 46/// /3510 11003 21019 30098 40110 58010="};
	 for(int i=0;i<3;i++){
		String s=ISHParser.parse(a[i]);	
	System.out.println(s);
	 }

		// logIt(fDebug, iPROD, false,
		// "---------------------------- Begin "+sProgramName); // Append output
		// to log.
		// logIt(fDebug, iPROD, false,
		// "Number of args found=["+args.length+"]"); // Append output to log.

		// Process args
	if (args.length <= 1) {
			bStdErr = true;
			logIt(fDebug, iPROD, false,
					"Error. Input and Output filenames required."); // Append
																	// output to
																	// log.
			System.exit(77);
		}
		...
		if (args.length >= 3) {
			if (args[2].equals("0") || args[2].equals("1")) {
				iLogLevel = Integer.parseInt(args[2]); // Safe to convert to
														// int.
			} else {
				logIt(fDebug, iPROD, false,
						"Invalid log message level parameter=[" + args[2]
								+ "].  Must be 0 or 1.  Defaulting to ["
								+ iLogLevel + "]");
			}
		}
		...
		logIt(fDebug, iDEBUG, false, "        Input Filename=[" + sInFileName
				+ "]"); // Append output to log.
		logIt(fDebug, iDEBUG, false, "       Output Filename=[" + sOutFileName
				+ "]"); // Append output to log.
		logIt(fDebug, iDEBUG, false, "         Logging Level=[" + iLogLevel
				+ "]"); // Append output to log.
		...
		try {
			BufferedReader fInReader = new BufferedReader(new FileReader(
					sInFileName));
			...
			try {
				...
				while ((line = fInReader.readLine()) != null) {
					...
					sMandatorySection = sMDS_Dir + " " + sMDS_Spd + " "
							...
							+ sGF1_Low + " " + sGF1_Med + " " + sGF1_Hi + " "
							+ sMDS_Vsb + " " + sWW1234[3] + " " + sWW1234[2]
							+ " " + sWW1234[1] + " " + sWW1234[0] + " "
							+ sAW1234[3] + " " + sAW1234[2] + " " + sAW1234[1]
							+ " " + sAW1234[0] + " " + sAY1_Pw + " "
							+ sMDS_Temp + " " + sMDS_Dewp + " " + sMDS_Slp
							+ " " + sMA1_Alt + " " + sMA1_Stp + " " + sMaxTemp
							+ " " + sMinTemp + " " + sPcp01 + sPcp01t + sPcp06
							+ sPcp06t + sPcp24 + sPcp24t + sPcp12 + sPcp12t
							+ sAJ1_Sd;
				...
				System.err.println(sProgramName + ": IOException 2. Error=["
						+ ex.getMessage() + "]");
				System.err.println(sProgramName + ": Stack trace follows:");
				ex.printStackTrace();
				System.exit(2);
	...
	public static void getMDS(String p_sRecd) {
		...
		if (sMDS_Vsb.equals("999999")) {
			...
			fWork = ((float) (fWork * (float) 0.000625)); // Convert Meters to
															// Miles using CDO's
															// value
		...
		if (sMDS_Slp.equals("99999")) {
			...
			fWork = ((float) (fWork / 10.0)); // Convert convert Hectopascals to
												// Millibars
	...
	public static void getMA1(String p_sRecd) {
		...
		if ((iMA1_IndexOf >= 0) && (iMA1_IndexOf < iREM_IndexOf)) {
			...
			if (sMA1_Alt.equals("99999")) {
				...
				try {
					fWork = Float.parseFloat(sMA1_Alt); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
			...
			if (sMA1_Stp.equals("99999")) {
				...
				try {
					fWork = Float.parseFloat(sMA1_Stp); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void getKA1(String p_sRecd) {
		...
		if ((iKA1_IndexOf >= 0) && (iKA1_IndexOf < iREM_IndexOf)) {
			...
			if (sKA1_Temp.equals("+9999")) {
				...
				try {
					fWork = Float.parseFloat(sKA1_Temp); // Convert to floating
															// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void getKA2(String p_sRecd) {
		...
		if ((iKA2_IndexOf >= 0) && (iKA2_IndexOf < iREM_IndexOf)) {
			...
			if (sKA2_Temp.equals("+9999")) {
				...
				try {
					fWork = Float.parseFloat(sKA2_Temp); // Convert to floating
															// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void getAA1(String p_sRecd) {
		...
		if ((iAA1_IndexOf >= 0) && (iAA1_IndexOf < iREM_IndexOf)) {
			...
			if (sAA1_Pcp.equals("9999")) {
				...
				try {
					fWork = Float.parseFloat(sAA1_Pcp); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void getAA2(String p_sRecd) {
		...
		if ((iAA2_IndexOf >= 0) && (iAA2_IndexOf < iREM_IndexOf)) {
			...
			if (sAA2_Pcp.equals("9999")) {
				...
				try {
					fWork = Float.parseFloat(sAA2_Pcp); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void getAA3(String p_sRecd) {
		...
		if ((iAA3_IndexOf >= 0) && (iAA3_IndexOf < iREM_IndexOf)) {
			...
			if (sAA3_Pcp.equals("9999")) {
				...
				try {
					fWork = Float.parseFloat(sAA3_Pcp); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iPROD,
	...
	public static void getAA4(String p_sRecd) {
		...
		if ((iAA4_IndexOf >= 0) && (iAA4_IndexOf < iREM_IndexOf)) {
			...
			if (sAA4_Pcp.equals("9999")) {
				...
				try {
					fWork = Float.parseFloat(sAA4_Pcp); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	public static void setPcp(String p_sHours, String p_sTrace) {
		fWork = (fWork / (float) 10.0) * (float) .03937008; // Convert precip
															// depths from
															// Millimeters to
															// Inches
	...
	public static void getAJ1(String p_sRecd) {
		...
		if ((iAJ1_IndexOf >= 0) && (iAJ1_IndexOf < iREM_IndexOf)) {
			...
			if (sAJ1_Sd.equals("9999")) {
				...
				try {
					fWork = Float.parseFloat(sAJ1_Sd); // Convert to floating
														// point
				} catch (Exception e) {
					logIt(fDebug,
							iDEBUG,
	...
	// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	public static int logIt(FileOutputStream p_fDebug, int p_iLogLevel,
		...
		if (p_bFilter) // 04/01/2009 ras
		{
			if (p_sFilter1.equals("None")) // 04/01/2009 ras
			{
				...
				if (sConcat.equals(p_sFilter1) || // 04/01/2009 ras // Life is
													// good
		...
		try {
			p_fDebug = new FileOutputStream(sDebugName + ".debug", true); // Append
																			// mode.
			new PrintStream(p_fDebug).println(format.format(now) + "_" + p_sIn); // Write
																					// output
																					// to
																					// debug
																					// log.
